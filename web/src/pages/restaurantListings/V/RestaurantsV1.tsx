...
import {fetchRestaurants} from '../../../features/restaurant/restaurantSlice/V/restaurantSlice_V';
import { selectAllRestaurants, selectFavorites, selectPaginatedRestaurants } from '../../../features/restaurant/restaurantSlice/V/restaurantSlice_V';

const Restaurants: React.FC = () => {
  const dispatch = useAppDispatch();
  const restaurrants:Restaurant[]=useAppSelector(selectAllRestaurants || []);
  ...
  //should remove after implementing persist on redux store 
  useEffect(() => {
    if(restaurrants.length === 0) {
      dispatch(fetchRestaurants());
    }
  }, [dispatch]);

  // Filter restaurants
  const filteredRestaurants = restaurrants.filter(restaurant => {
    // Search filter
        ...
        <Grid item xs={12} md={9}>
          ...
          {currentRestaurants.length === 0 ? (
            ...
            <>
              ...
              Pagination
              {totalPages > 1 && (
                <Box sx={{ display: 'flex', justifyContent: 'center', mt: 6, sticky: 'bottom', py: 2, backgroundColor: 'background.paper' }}>
                  <Pagination
                    count={totalPages}
                    page={currentPage}
