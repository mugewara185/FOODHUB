  ...
  try {
    ...
    if (targets.includes('orders')) {
      for (const order of orders) {
        const resolvedUserId = createdUserMap.get(order.userId) || createdUsers[0]?.id || order.userId;
        const resolvedRestaurantId = createdRestaurantMap.get(order.restaurantId) || createdRestaurants[0]?.id || order.restaurantId;
    ...
    if (targets.includes('reviews')) {
      for (const review of reviews) {
        const resolvedUserId = createdUserMap.get(review.userId) || createdUsers[0]?.id || review.userId;
        const resolvedRestaurantId = createdRestaurantMap.get(review.restaurantId) || createdRestaurants[0]?.id || review.restaurantId;
